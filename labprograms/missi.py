...
def is_state_safe(state):
    otherstate=[start[i]- state[i]for i in range(3)]
    
    if is_bank_safe(state) and is_bank_safe(otherstate) :
        return True
    else:
        return False
...
def solve(next_action,path):
    ...
    if next_action==end:
        ...
        solution.append(_path)
        return
    elif next_action in end:
        return
    else:
